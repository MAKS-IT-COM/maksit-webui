...
<#
.SYNOPSIS
    Helm publish plugin — package and push charts versioned from shared release context.

.DESCRIPTION
    The chart in the repo should keep placeholder version and appVersion (e.g. 0.0.0); this plugin
    overwrites them with the bare semver from shared context (shared.version from a release-version
    plugin, e.g. DotNetReleaseVersion or NpmReleaseVersion — no leading v), falling back to stripping
    v/V from shared.tag if version is missing, then runs helm package and helm push, then restores Chart.yaml.
...
function Invoke-Plugin {
    ...
    if ([string]::IsNullOrWhiteSpace($pluginSettings.chartPath)) {
        throw "HelmPush plugin requires 'chartPath' (chart directory, relative to engines/release folder)."
    }
    ...
    if (-not $dryRun) {
        ...
        if ([string]::IsNullOrWhiteSpace($containerRegistrySecret)) {
            throw "HelmPush plugin requires 'containerRegistrySecret' (logical secret name, e.g. ContainerRegistry)."
        }
    }
    ...
    if ([string]::IsNullOrWhiteSpace($chartVersion)) {
        throw "Could not derive chart version: need shared.version (release version plugin) or shared.tag (e.g. v3.3.4)."
    }
    ...
    try {
        ...
        if ($dryRun) {
            Write-Log -Level "INFO" -Message "Dry run: would push $chartPackage to $ociRepository"
            Remove-Item -LiteralPath $chartPackage -Force -ErrorAction SilentlyContinue
            Write-Log -Level "OK" -Message "  Helm chart dry run completed."
            return
        }
