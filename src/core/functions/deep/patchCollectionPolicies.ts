 ...
 *
 * Combines `entityId` and `entityType` so one scope row exists per entity+type
 * during {@link deepDelta} array diffing (scope is updated in place).
 */
const entityScopeFallback = (item: Record<string, unknown>): string => {
 ...
 */
const versionFallback = (item: Record<string, unknown>): string => {
