...
import { deepDelta, deltaHasOperations } from '@webui/core/functions/deep/deepDelta'
import {
  ENTITY_SCOPES_ARRAY_POLICY,
  HOSTNAMES_ARRAY_POLICY,
  VERSIONS_ARRAY_POLICY,
} from '@webui/core/functions/deep/patchCollectionPolicies'
...
describe('deepDelta', () => {
  describe('primitive fields', () => {
    ...
    it('marks nullish values as RemoveField', () => {
      ...
      const delta = deepDelta<{ name: string; optional: string | null }>(form, backup)
  ...
  describe('ENTITY_SCOPES_ARRAY_POLICY edge cases', () => {
    ...
    /**
     * Synthetic identity is entityId + entityType (scope is a field), so changing scope
     * on a row without server `id` is an in-place update (not remove + add).
     *
     * backup:
     ...
     *   { entityScopes: [{ entityId: 'org-1', entityType: 0, scope: 3 }] }
     */
    it('updates scope bitmask on id-less rows in place by _deltaId', () => {
      const delta = deepDelta(
